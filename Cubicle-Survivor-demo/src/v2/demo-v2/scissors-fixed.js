...
(function () {
  ...
  function normalizeState(state) {
    ...
    if (state.stage) {
      ...
      state.stage.phase = "Demo V2.3 剪刀固定测试 · 阶段 " + (test.currentPhase || 1);
      if (test.collecting) state.stage.name = "第 " + (state.stage.id || 1) + "/17 关 · 资源回收";
    }
    if (state.flags && state.flags.won) {
      state.lastRewardReason = "剪刀 Demo V2.3 完成：近战时间线、轻步闪身、合刃/张刃路线、组件取舍与低血保护罩均已完成固定测试。";
    }
  }
  ...
  function rebuildParams(state) {
    ...
    const damage = 24 * Math.pow(1.05, experience.damage || 0) * Math.pow(1.15, blade.damage || 0);
    const speedScale = Math.max(0.55, Math.pow(0.9, pivot.attackSpeed || 0) * Math.pow(0.96, experience.attackSpeed || 0));
    const rangeScale = Math.min(1.38, Math.pow(1.09, handle.range || 0) * (1 + (experience.range || 0) * 0.025));
    ...
    const cooldown = Math.max(0.42, actionDuration + 0.28 * speedScale);
    const maxHp = 78 + (experience.maxHp || 0) * 10;

    state.activeFormParams = {
      damage,
      cooldown,
      range: Math.min(205, (150 + closedLevel * 6) * rangeScale),
      width: 26 * rangeScale,
      amount: 1,
      pierce: 99,
      markerFixedHpRegen: (experience.hpRegen || 0) * 0.65,
      ...
      scissorsThrustRange: Math.min(205, (150 + closedLevel * 6) * rangeScale),
      scissorsThrustWidth: Math.min(42, 24 * rangeScale + closedLevel * 2),
      scissorsThrustDamage: damage * 1.08,
      scissorsFanRange: Math.min(148, (102 + openLevel * 5) * rangeScale),
      scissorsFanHalfAngle: Math.min(0.72, 0.48 + openLevel * 0.045 + (rangeScale - 1) * 0.08),
      scissorsFanDamage: damage * 0.5,
      scissorsSeverRange: Math.min(255, 205 * rangeScale),
      scissorsSeverWidth: Math.min(72, 50 * rangeScale),
      ...
      scissorsDashDistance: 122,
      scissorsDashWindow: 0.24,
      scissorsDashChargeTime: 4.2,
      scissorsDashRoundCharge: 0.22,
      scissorsShelterRadius: 108,
  ...
  function makeExperienceChoices(state) {
    return (callBase(state, "makeExperienceChoices", [state]) || []).map(function (choice) {
      if (choice.id === "damage") choice.effect = "所有剪刀动作伤害 +5%";
      if (choice.id === "range") choice.effect = "近战距离与宽度小幅提高（受近战上限限制）";
      if (choice.id === "moveSpeed") choice.effect = "移动速度 +3%（不改变轻步距离）";
      return choice;
    });
  }
  ...
  function tick(state, dt) {
    ...
    if (!test.collecting && !test.dashReady) {
      test.dashCharge = Math.min(1, (test.dashCharge || 0) + delta / Math.max(1, params.scissorsDashChargeTime || 4.2));
  ...
  function onRoundComplete(state, uniqueTargets) {
    ...
    const params = state.activeFormParams || {};
    if (!test) return;
    const multiBonus = Math.min(0.12, Math.max(0, (uniqueTargets || 0) - 1) * 0.025);
    test.dashCharge = Math.min(1, (test.dashCharge || 0) + (params.scissorsDashRoundCharge || 0.22) + multiBonus);
    if (test.dashCharge >= 1) test.dashReady = true;
  }
  ...
  const scissorsFixed = {
    ...
    uiFramework: {
      weaponSelection: { activeIds: ["scissors"], cardCapacity: 6, registryLabel: "当前验证武器" },
