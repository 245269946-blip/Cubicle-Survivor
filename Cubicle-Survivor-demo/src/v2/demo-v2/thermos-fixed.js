...
(function () {
  ...
  function normalizeState(state) {
    ...
    if (state.stage) {
      ...
      state.stage.phase = "Demo V2.2 保温杯固定测试 · 阶段 " + (test.currentPhase || 1);
      if (test.collecting) state.stage.name = "第 " + (state.stage.id || 1) + "/17 关 · 资源回收";
    }
    if (state.flags && state.flags.won) {
      state.lastRewardReason = "保温杯 Demo V2.2 完成：近距扇面、冷凝铺场、聚焦击杀热浪与组件取舍均已完成固定测试。";
    }
  }
  ...
  function rebuildParams(state) {
    ...
    const damage = 14 * Math.pow(1.05, experience.damage || 0) * Math.pow(1.15, lid.damage || 0);
    const rangeScale = Math.pow(1.1, cupBase.range || 0);
    state.activeFormParams = Object.assign({}, state.activeFormParams, {
      damage,
      cooldown: 1.28 * Math.pow(0.88, body.attackSpeed || 0) * Math.pow(0.95, experience.attackSpeed || 0),
      range: 225 * rangeScale + (experience.range || 0) * 10,
  ...
  function makeExperienceChoices(state) {
    const choices = callBase(state, "makeExperienceChoices", [state]) || [];
    return choices.map(function (choice) {
      if (choice.id === "damage") choice.effect = "所有保温杯伤害 +5%";
      if (choice.id === "range") choice.effect = "扇面距离与宽度小幅提高";
      return choice;
    });
  }
  ...
  const thermosFixed = {
    ...
    uiFramework: {
      weaponSelection: { activeIds: ["thermos"], cardCapacity: 6, registryLabel: "当前验证武器" },
