...
// One state machine for the active Steam Demo loop.
// ================================================================
(function () {
  ...
  const STAGE_BLUEPRINTS = [
    { id: 1, phaseKey: "weapon_intro", phaseStep: 1, phase: "实习期", name: "工具热身", duration: 38, targetKills: 24, spawnEvery: 1.25, enemyHp: 18, enemySpeed: 58, material: 7, enemyMix: [{ type: "todo", weight: 10 }], note: "先体验初始武器的基础清怪方式。", threatHint: "基础清怪：看懂马克笔、保温杯、即时贴各自的攻击动词。" },
    { id: 2, phaseKey: "weapon_intro", phaseStep: 2, phase: "实习期", name: "工位熟悉", duration: 44, targetKills: 32, spawnEvery: 1.08, enemyHp: 21, enemySpeed: 63, material: 8, enemyMix: [{ type: "todo", weight: 7 }, { type: "email", weight: 3 }], note: "用经验和材料微调这把武器，但还不改变部门形态。", threatHint: "移动配合：用走位把敌人带进武器喜欢的形状。" },
    { id: 3, phaseKey: "weapon_intro", phaseStep: 3, phaseFinal: true, phase: "实习期", name: "实习小考", duration: 50, targetKills: 1, boss: true, bossType: "lead", spawnEvery: 1.5, enemyHp: 360, enemySpeed: 50, material: 10, enemyMix: [{ type: "todo", weight: 6 }, { type: "email", weight: 2 }], note: "完成实习后再选择工牌，进入转正阶段。", threatHint: "基础爆发：确认初始武器能否处理厚血目标。" },
    { id: 4, phaseKey: "promotion", phaseStep: 1, phase: "转正期", name: "工牌定型", duration: 52, targetKills: 40, spawnEvery: 1.02, enemyHp: 24, enemySpeed: 66, material: 11, enemyMix: [{ type: "todo", weight: 6 }, { type: "meeting", weight: 3 }, { type: "email", weight: 2 }], note: "工牌让同一把武器变成新的战斗形态。", threatHint: "形态识别：观察工牌如何改写主武器打法。" },
    { id: 5, phaseKey: "promotion", phaseStep: 2, phase: "转正期", name: "职责加压", duration: 56, targetKills: 48, spawnEvery: 0.94, enemyHp: 28, enemySpeed: 70, material: 12, enemyMix: [{ type: "todo", weight: 5 }, { type: "ping", weight: 3 }, { type: "meeting", weight: 2 }], note: "卡槽开始围绕当前形态做取舍。", threatHint: "短板暴露：远程群消息会逼你移动，厚会议怪会检验单体。" },
    { id: 6, phaseKey: "promotion", phaseStep: 3, phase: "转正期", name: "流程压测", duration: 60, targetKills: 56, spawnEvery: 0.86, enemyHp: 32, enemySpeed: 74, material: 14, enemyMix: [{ type: "todo", weight: 4 }, { type: "deadline", weight: 3 }, { type: "ping", weight: 2 }, { type: "meeting", weight: 2 }], note: "继续补强主形态，准备转正评审。", threatHint: "空间压测：冲刺截止日期会打断站桩，验证移动容错。" },
    { id: 7, phaseKey: "promotion", phaseStep: 4, phaseFinal: true, phase: "转正期", name: "转正评审", duration: 68, targetKills: 1, boss: true, bossType: "director", spawnEvery: 1.5, enemyHp: 760, enemySpeed: 56, material: 22, enemyMix: [{ type: "todo", weight: 4 }, { type: "ping", weight: 2 }], note: "通过评审后，主武器获得专属转正强化。", threatHint: "成型检查：评审目标带护盾，第一套主轴必须能处理厚血目标。" },
    { id: 8, phaseKey: "promoted_mastery", phaseStep: 1, phase: "独当一面", name: "主轴复盘", duration: 62, targetKills: 66, spawnEvery: 0.78, enemyHp: 36, enemySpeed: 78, material: 16, enemyMix: [{ type: "todo", weight: 4 }, { type: "scope", weight: 3 }, { type: "deadline", weight: 2 }], note: "围绕转正强化继续堆主形态，但还保留成长空间。", threatHint: "持续输出：需求变更死亡后会拆成小待办，检验连锁清怪。" },
    { id: 9, phaseKey: "promoted_mastery", phaseStep: 2, phase: "独当一面", name: "单人负责", duration: 66, targetKills: 72, spawnEvery: 0.72, enemyHp: 40, enemySpeed: 82, material: 18, enemyMix: [{ type: "deadline", weight: 3 }, { type: "approval", weight: 3 }, { type: "ping", weight: 2 }, { type: "todo", weight: 2 }], note: "主工牌打法开始稳定成型，工坊和卡槽只补强当前主轴。", threatHint: "补短板：审批流抗伤更高，远程消息继续压走位。" },
    { id: 10, phaseKey: "promoted_mastery", phaseStep: 3, phaseFinal: true, phase: "独当一面", name: "独立交付", duration: 76, targetKills: 1, boss: true, bossType: "delivery", spawnEvery: 1.4, enemyHp: 1180, enemySpeed: 60, material: 26, enemyMix: [{ type: "deadline", weight: 3 }, { type: "scope", weight: 2 }], note: "完成独立交付后，开放第二部门的初级专属能力。", threatHint: "主轴验收：交付目标会冲刺压迫，验证转正强化是否值得继续构筑。" },
    { id: 11, phaseKey: "cross_department", phaseStep: 1, phase: "跨部门协作", name: "协作磨合", duration: 66, targetKills: 78, spawnEvery: 0.68, enemyHp: 44, enemySpeed: 86, material: 20, enemyMix: [{ type: "todo", weight: 3 }, { type: "client", weight: 3 }, { type: "approval", weight: 2 }, { type: "email", weight: 2 }], note: "主武器保留主工牌打法，并叠加第二部门的初级形态。", threatHint: "协作入口：客户追问会拉开距离开火，第二部门只补一条能力。" },
    { id: 12, phaseKey: "cross_department", phaseStep: 2, phase: "跨部门协作", name: "联合推进", duration: 70, targetKills: 86, spawnEvery: 0.64, enemyHp: 48, enemySpeed: 90, material: 22, enemyMix: [{ type: "scope", weight: 3 }, { type: "client", weight: 3 }, { type: "deadline", weight: 2 }, { type: "approval", weight: 2 }], note: "观察两个部门形态如何互相补位，而不是追求单一路线封顶。", threatHint: "协同验证：分裂、远程、冲刺同时出现，看主形态和第二部门是否补位。" },
    { id: 13, phaseKey: "cross_department", phaseStep: 3, phaseFinal: true, phase: "跨部门协作", name: "联合评审", duration: 82, targetKills: 1, boss: true, bossType: "client", spawnEvery: 1.35, enemyHp: 1500, enemySpeed: 62, material: 30, enemyMix: [{ type: "client", weight: 3 }, { type: "approval", weight: 2 }], note: "完成联合评审后，开放跨技能学习。", threatHint: "双部门验收：Boss 会远程追问，主轴、副形态和生存容错都要交卷。" },
    { id: 14, phaseKey: "cross_weapon", phaseStep: 1, phase: "跨技能学习", name: "技能借调", duration: 70, targetKills: 88, spawnEvery: 0.62, enemyHp: 52, enemySpeed: 92, material: 24, enemyMix: [{ type: "deadline", weight: 3 }, { type: "scope", weight: 3 }, { type: "client", weight: 2 }, { type: "approval", weight: 2 }], note: "副武器只保留本质技能作为辅助，不抢主武器的 Build 主轴。", threatHint: "副武器入口：借用技能应能补一条短板，而不是重新洗构筑。" },
    { id: 15, phaseKey: "cross_weapon", phaseStep: 2, phase: "跨技能学习", name: "组合压测", duration: 76, targetKills: 96, spawnEvery: 0.58, enemyHp: 58, enemySpeed: 96, material: 26, enemyMix: [{ type: "deadline", weight: 3 }, { type: "client", weight: 3 }, { type: "approval", weight: 3 }, { type: "scope", weight: 2 }], note: "用经验和材料继续补属性，最终形态仍留有下一局可调整空间。", threatHint: "完整压测：清场、单体、生存、经济都开始同时吃紧。" },
    { id: 16, phaseKey: "cross_weapon", phaseStep: 3, phaseFinal: true, phase: "跨技能学习", name: "最终形态验证", duration: 88, targetKills: 1, boss: true, bossType: "ceo", spawnEvery: 1.3, enemyHp: 1880, enemySpeed: 66, material: 36, enemyMix: [{ type: "client", weight: 3 }, { type: "deadline", weight: 2 }, { type: "approval", weight: 2 }], note: "完成三层进化链路：主形态强化、第二部门、跨技能辅助。", threatHint: "终局验收：完整 Build 必须证明每个阶段选择都有意义。" }
  ];
