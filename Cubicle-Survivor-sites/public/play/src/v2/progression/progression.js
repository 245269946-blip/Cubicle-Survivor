...
(function () {
  ...
  const SLOT_DEFS = {
    offense: { name: "输出槽", unlock: 1, accent: "#ff6b4a", role: "提高主形态清场速度与命中收益。" },
    survival: { name: "生存槽", unlock: 2, accent: "#4acf6a", role: "把部分命中转成护盾、回复或控场。" },
    resource: { name: "资源槽", unlock: 2, accent: "#ffd700", role: "让主形态额外产出经验与材料。" },
    mechanic: { name: "机制槽", unlock: 3, accent: "#4a9eff", role: "改写分裂、回弹、留场或连线规则。" },
    cost: { name: "代价槽", unlock: 4, accent: "#f04040", role: "大幅强化主形态，但附带停火或承伤风险。" }
  };
  ...
  const UPGRADE_POOL = [
    ...
    },
    {
      ...
      apply(state) {
        ...
        p.range = Math.round((p.range || 320) * 1.1);
        if (p.steamRange) p.steamRange = Math.round(p.steamRange * 1.1);
        if (p.releaseRange) p.releaseRange = Math.round(p.releaseRange * 1.1);
      }
    },
    {
      id: "area",
      ...
      apply(state) {
        ...
        if (p.gridRadius) p.gridRadius = Math.round(p.gridRadius * 1.15);
        if (p.stationRadius) p.stationRadius = Math.round(p.stationRadius * 1.15);
        if (p.spreadRadius) p.spreadRadius = Math.round(p.spreadRadius * 1.15);
        if (p.teaRadius) p.teaRadius = Math.round(p.teaRadius * 1.15);
      }
    },
    {
      id: "speed",
  ...
  const SLOT_RULES = {
    line_split: {
      offense: { text: "每个主线命中点可多锁定 1 个分裂目标，支线仍按目标方向贯穿。", apply: p => { p.splitCount = (p.splitCount || 1) + 1; p.damage = Math.round((p.damage || 10) * 1.08); } },
      ...
      mechanic: { text: "支线可再次锁定一个尚未命中的目标，但二次分裂伤害递减。", apply: p => { p.secondarySplit = true; } },
    ...
    shield_counter_line: {
      ...
      resource: { text: "护盾反刺击杀时额外掉材料。", apply: p => { p.materialBonus = (p.materialBonus || 0) + 0.2; } },
  ...
  const PROMOTION_RULES = {
    line_split: {
      ...
      apply(p) {
        ...
        p.promotionFullscreenEvery = Math.min(p.promotionFullscreenEvery || 6, 6);
      }
    },
    mark_detonate: {
      title: "转正强化：P0 扩大爆破",
    ...
    charge_release_beam: {
      ...
      apply(p) {
        p.releaseDamage = Math.round((p.releaseDamage || 68) * 1.22);
  ...
  const MASTERY_RULES = {
    line_split: {
      ...
      apply(p) { p.splitCount = (p.splitCount || 2) + 1; p.splitDamage = (p.splitDamage || 0.42) + 0.08; }
    },
    mark_detonate: {
      title: "主轴精修：连环 P0",
    ...
    line_to_wave: {
      ...
      apply(p) { p.waveCount = (p.waveCount || 1) + 1; }
    },
    line_grid_field: {
      title: "主轴精修：审批留痕",
    ...
    charge_release_beam: {
      ...
      apply(p) { p.heatRate = Math.round((p.heatRate || 24) * 1.1); p.releaseWidth = (p.releaseWidth || 22) + 8; p.releaseLockoutDuration = (p.releaseLockoutDuration || 1.2) + 0.2; }
    },
    shield_break_pulse: {
      title: "主轴精修：二段暖流",
  ...
  function formSpecificOffers(state) {
    ...
    const special = {
      ...
      line_split: [
        offer("shop_split_count", "增加锁敌分支", baseCost + 3, "每个命中点可多锁定 1 个新目标", "只有附近存在未被主线命中的敌人时，才会生成真实伤害支线。", s => { s.activeFormParams.splitCount = (s.activeFormParams.splitCount || 1) + 1; }),
      ...
      line_to_wave: [
        offer("shop_wave_count", "追加一次传播波", baseCost + 3, "第一道波前后再扩散一次", "第二道波纹会延迟出现，并独立扫过敌人结算伤害。", s => { s.activeFormParams.waveCount = (s.activeFormParams.waveCount || 1) + 1; }),
  ...
  function getRefreshCost(state) {
    const stageId = state.stage && state.stage.id ? state.stage.id : 1;
    return 3 + (state.shopRerolls || 0) * 2 + Math.floor(stageId / 6);
  }

  function annotateOffer(item, category) {
    item.category = category;
    item.categoryLabel = SHOP_CATEGORY_LABELS[category] || "工坊强化";
    return item;
  }

  function inferOfferCategory(item) {
    if (/recover|shield|station_heal|route_shield|warm_shield|补|护盾|容错|安全|回复|补给|减速/.test(item.id + item.title)) return "patch";
    return "core";
  }

  function makeRiskOffer(state) {
    const p = state.activeFormParams || {};
    const stageId = state.stage && state.stage.id ? state.stage.id : 1;
    const baseCost = 10 + stageId;
    return annotateOffer(offer(
      "shop_high_pressure_" + stageId,
      "高压加班模式",
      baseCost,
      "伤害 " + Math.round(p.damage || 0) + " → " + Math.round((p.damage || 10) * 1.32) + "，间隔略变长",
      "用更高输出赌下一关能快速清场，但会留下更明显的攻击空窗。",
      s => {
        s.activeFormParams.damage = Math.round((s.activeFormParams.damage || 10) * 1.32);
        s.activeFormParams.cooldown = Math.max(0.35, (s.activeFormParams.cooldown || 1.4) * 1.08);
        s.activeFormParams.risk = (s.activeFormParams.risk || 0) + 1;
      }
    ), "risk");
  }
  ...
  function makeShopOffers(state) {
    const rerolls = state.shopRerolls || 0;
    const offers = formSpecificOffers(state).map(function (item) {
      return annotateOffer(item, inferOfferCategory(item));
    });
    const core = offers.filter(function (item) { return item.category === "core"; });
    const patch = offers.filter(function (item) { return item.category === "patch"; });
    const risk = makeRiskOffer(state);
    return [
      pickRotated(core, rerolls),
      pickRotated(patch.length ? patch : core, rerolls + 1),
      risk
    ].filter(Boolean);
  }
