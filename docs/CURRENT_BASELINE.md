...
- Demo V3.6 adds `demo-v3-6.html` as the current recommended four-weapon entry. It preserves the complete Demo V3.5 balance and encounter package, then packages the Marker desire-loop display contract into the fixed suite: the character and parallel Copy/Archive cartridges follow body direction, the physical pen follows the real attack aim, Demo amount and Copy level determine visible pen count, and module levels drive separate yellow/cyan core and bloom intensity. See `docs/DEMO_V3_6_MARKER_EMBODIMENT_PASS.md`.
- Demo V3.7 adds `demo-v3-7.html` as the current recommended four-weapon entry. It preserves the complete Demo V3.6 combat and Marker-embodiment package, then applies the same ownership contract to Thermos: a shared body-facing pressure rack carries separate cyan Condensation and amber Kill-Heatwave packs, while only the near-body cup/nozzle follows real attack aim. Module levels add directional wearable packs and monotonic route glow; visible steam begins at the real cup outlet while the hidden body-to-cup segment preserves close-range judgment. See `docs/DEMO_V3_7_THERMOS_PRESSURE_RIG_PASS.md`.
...
- Demo V3.13 adds `demo-v3-13.html` as the current recommended four-weapon entry. It preserves V3.12 opening numbers and all 17 encounter/economy rules, then extends the same readable growth contract to the remaining weapons with one concise relation each: Thermos Heatwave × Condensation creates Thermal Exchange, Scissors Open × Closed creates Cross Cut, and Correction Fluid overload deaths trigger Cascading Rollback through old error areas. All three retain a fifth-choice pure Lv4 mastery and weapon-specific physical component mounts. See `docs/DEMO_V3_13_ALL_WEAPON_DESIRE_CHAINS.md`.
