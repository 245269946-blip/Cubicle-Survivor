...
- 2026-08-27 GitHub Pages reliability hotfix: V3.15 no longer starts by requesting the entire roughly 135-image / 75 MB runtime library. The landing and four-weapon choice open immediately; choosing a weapon then gates combat on 10 shared first-encounter assets plus only that weapon's 3 rig/VFX assets. Loading is capped at 6 concurrent images with up to 4 timeout/network retries; later formal assets prefetch only after the chosen 13 are ready, while legacy regression sprites remain on demand. A missing character/weapon/background therefore cannot silently enter play. Local browser verification covers the two-stage gate, all four weapon base combat states and the compact component shop with zero runtime errors. The weapon page is now image + name + one attack label only; the component shop keeps part, stat, immediate result, cost and action, with process strips and mounting prose removed in compact V3.15.
