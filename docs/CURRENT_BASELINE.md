...
- 2026-08-27 GitHub Pages reliability hotfix: V3.15 no longer starts by requesting the entire roughly 135-image / 75 MB runtime library. It gates play on 22 first-encounter assets, loads at most 6 images concurrently, retries timeout/network failures up to 4 times, then prefetches formal later-stage assets and leaves legacy regression sprites on demand. The start button exposes real loading or retry state, so a missing character/weapon/background cannot silently enter play. Local browser verification covers the gate, all four weapon base combat states and the compact component shop with zero runtime errors. The weapon page is now image + name + one attack label only; the component shop keeps part, stat, immediate result, cost and action, with process strips and mounting prose removed in compact V3.15.
