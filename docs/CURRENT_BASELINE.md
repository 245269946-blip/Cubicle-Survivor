...
- Demo V2.7 is a preserved four-weapon playable snapshot. It repairs the real stage path, advances the first module choice to encounter 1, separates Boss health from normal-add health, and completes the Scissors dash/readability and fixed-suite UI pass documented in `docs/DEMO_V2_7_PLAYABLE_EXPERIENCE_FIX.md`.
- Demo V2.8 and Demo V2.9 remain preserved regression snapshots for combat tempo and horizontal consistency.
- Demo V3.0 is the preserved combat-perception snapshot over Demo V2.9: no new item or enemy system, but complete hit/lock/defeat feedback, readable Correction Fluid states, short growth confirmations, Scissors risk correction, and a Japanese neon-city information layer across all player-visible screens. See `docs/DEMO_V3_0_COMBAT_PERCEPTION_PASS.md`.
- Demo V3.1 is the preserved density snapshot. It redistributes weapon output into smaller, faster events; raises fixed-encounter enemy floors, batches and quotas; projects the Scissors dash intent beyond the held weapon; and separates Thermos Kill Heatwave from Condensation through a hot core, expanding amber front and pressure echo. See `docs/DEMO_V3_1_COMBAT_DENSITY_PASS.md`.
- Demo V3.2 is the preserved combat-triangle snapshot. It deepens the same three-part combat budget—smaller strikes, shorter attack gaps and a higher effective-target floor—while adding low-opacity outer bloom plus a sharp inner core to real weapon events. See `docs/DEMO_V3_2_COMBAT_TRIANGLE_NEON_PASS.md`.
- Demo V3.3 is the preserved Correction Fluid opening snapshot: slightly stronger/faster primary spray plus one weak nearby overspray until Fatal Correction supplies true independent multi-target locks. See `docs/DEMO_V3_3_CORRECTION_OPENING_PASS.md`.
- Demo V3.4 is the preserved encounter-space snapshot. It moves the run start to the world centre, randomizes wave entry across the full view perimeter, and adds telegraphed Boss lane and safe-gap barrage attacks. See `docs/DEMO_V3_4_ENCOUNTER_SPACE_PASS.md`.
- Demo V3.5 adds `demo-v3-5.html` as the preserved sustained-pressure regression entry. It distributes each fixed enemy quota across the full encounter, accelerates enemy approach and active attacks, restores ordinary Boss intent between special patterns, raises Boss durability and cadence, and makes component attributes visibly enlarge real attack shapes. It also separates Marker laser/ink color hierarchy and attaches the ordinary Scissors model to the leading edge of Closed-Blade attacks. See `docs/DEMO_V3_5_SUSTAINED_PRESSURE_PASS.md`.
- Demo V3.6 is the preserved Marker embodiment snapshot. It packages the Marker desire-loop display contract into the fixed suite: the character and parallel Copy/Archive cartridges follow body direction, the physical pen follows the real attack aim, Demo amount and Copy level determine visible pen count, and module levels drive separate yellow/cyan core and bloom intensity. See `docs/DEMO_V3_6_MARKER_EMBODIMENT_PASS.md`.
- Demo V3.7 is the preserved Thermos pressure-rig snapshot. A shared body-facing pressure rack carries separate cyan Condensation and amber Kill-Heatwave packs, while only the near-body cup/nozzle follows real attack aim. See `docs/DEMO_V3_7_THERMOS_PRESSURE_RIG_PASS.md`.
- Demo V3.9 remains the previous Scissors/Correction embodiment snapshot. It preserves the complete Demo V3.8 combat snapshot, aligns Scissors and Correction Fluid to the same chibi office-worker identity and 112px combat height used by Marker/Thermos, gives Scissors one authoritative complete-weapon animation, and makes Correction Fluid visibly travel from a worn reservoir through attached nozzles into white enemy error marks. See `docs/DEMO_V3_9_SCISSORS_CORRECTION_EMBODIMENT_PASS.md`.
- Demo V3.10 is the preserved battlefield-scale snapshot. All four characters use a compact 78px draw height with an approximately 55—59px visible footprint, body-mounted equipment stays attached to body-facing sockets, and only the real aiming pen, cup, complete scissors or correction nozzle follows attack aim on a weapon-specific outer ring. See `docs/DEMO_V3_10_COMBAT_SCALE_ORBIT_PASS.md` and `docs/WEAPON_VISUAL_DESIGN_STANDARD.md`.
- Demo V3.11 remains the opening-comfort and weapon-parity regression snapshot. It preserves V3.10 presentation and progression, applies an onboarding comfort envelope only to encounters 1–2, trims Marker/Thermos opening dominance, fixes Scissors real-range acquisition and melee survival, and accelerates Correction Fluid's first error-state payoff. See `docs/DEMO_V3_11_OPENING_COMFORT_PARITY_PASS.md`.
- Demo V3.12 remains the Marker-only desire-chain regression snapshot. It preserves V3.11 combat numbers and turns Marker growth into a visible desire chain. See `docs/DEMO_V3_12_MARKER_DESIRE_CHAIN_PASS.md`.
- Demo V3.13 remains the four-weapon desire-chain regression snapshot. It preserves V3.12 opening numbers and all 17 encounter/economy rules, then extends the same readable growth contract to the remaining weapons with Thermal Exchange, Cross Cut and Cascading Rollback. See `docs/DEMO_V3_13_ALL_WEAPON_DESIRE_CHAINS.md`.
- Demo V3.14 adds `demo-v3-14.html` as the preserved compact-decision regression snapshot. It preserves every V3.13 combat, progression and economy rule, and compresses weapon, experience, module and component decision pages to name + immediate result + at most one future/relationship cue. See `docs/DEMO_V3_14_DECISION_DENSITY_PASS.md`.
- Demo V3.15 adds `demo-v3-15.html` as the current formal four-weapon release. It preserves V3.14 combat, 17 encounters, progression, economy and compact-decision rules, then enables only independently gated cartoon assets. The formal set is `runtime-ready`: 8 normal enemies (48 action/facing cases), 5 Bosses (52 combat/HUD cases), 5 office scenes, 3 four-frame pickups, compact HUD, four real-alpha weapon VFX atlases and 21 decoded PCM WAV cues. The Scissors v3 atlas keeps complete rings, pivot and blades in every Closed/Open frame; GitHub Pages evidence covers all eight states with zero runtime errors. A 0.82-second text-free completion check plus encounter/final cues expresses “finishing a task” independently of weapon growth. Choice and purchase pages keep name + immediate result + at most one future/relationship cue at readable type sizes. Generated images remain source material until target geometry, runtime wiring, judgment alignment and browser evidence pass; quota or service failure must remain blocked rather than downgrade to line-art placeholders. V3.14 and older entries remain regression snapshots. See `docs/DEMO_V3_15_CARTOON_ASSET_PRODUCTION.md`, `Cubicle-Survivor-demo/formal-cartoon-vfx-runtime-report.json` and `Cubicle-Survivor-demo/formal-cartoon-audio-runtime-report.json`.
- 2026-08-25 Scissors VFX revision: V3.15 wires `scissors-vfx-v3.png`, an image-generated real-alpha 4×2 atlas that restores one complete two-ring, central-pivot, two-blade scissors in every Closed/Open frame. It replaces the abstract paper/X-heavy scissors v2 runtime asset while preserving real event geometry and all combat numbers. Source preservation, chroma removal, deterministic normalization, exact 1024×512 dimensions, target-size office composite and `npm test` pass. GitHub Pages fixed-state evidence now covers Closed/Open frames 0—3 with the v3 asset, correct geometry/event sources and zero runtime errors; the earlier v2 hashes were not reused.
- 2026-08-28 GitHub Pages reliability hotfix: V3.15 no longer starts by requesting the entire roughly 135-image / 75 MB runtime library. The landing and four-weapon choice open immediately; choosing a weapon then gates combat on 7 shared first-frame assets plus only that weapon's 3 rig/VFX assets. Loading is capped at 6 concurrent images with up to 4 timeout/network retries; later formal assets (including pickups) prefetch only after the chosen 10 are ready, while legacy regression sprites remain on demand. A missing character/weapon/background therefore cannot silently enter play, and the choice page continuously shows real `X/10` progress. Local browser verification covers the two-stage gate, all four weapon base combat states and the compact component shop with zero runtime errors. The weapon page is now image + name + one attack label only; the component shop keeps part, stat, immediate result, cost and action, with process strips and mounting prose removed in compact V3.15.
- `Cubicle-Survivor-demo/cartoon-marker-slice.html` is an isolated P0 visual-and-completion experiment approved after Demo V3.14. It does not replace the recommended four-weapon entry or change its balance. The slice tests one 30-item backlog task, one Marker Copy upgrade expressed as “多画一条”, a unified paper-office cartoon style, enemy/weapon motion feedback and an explicit “已归档” completion event. Its arena, aim-driven Marker, shared neutral worker plus Marker back/front rig, both legacy enemy action atlases, four-frame backlog walk, five-frame backlog slam, four-frame email run and five-frame email dash are `runtime-ready`. P0 now uses the same four-direction `idle / step-a / step-b` identity as the authoritative character system rather than its old single-frame cutouts. Generated strips must be treated as source material and split by actual alpha-connected poses rather than nominal equal columns because limbs and props may cross model-implied cell boundaries. Browser evidence covers 32 distinct player/enemy/facing poses, 8 legacy enemy action poses, an upgraded live run and a slam timing probe proving damage occurs on impact rather than anticipation; the main shared character system separately retains its 640-case gate. See `docs/CHARACTER_WALK_STATE_MATRIX.md` and `docs/CARTOON_MARKER_P0_SLICE.md`.
- Demo V3.8 remains the previous Thermos back-pressure snapshot. It preserves the complete Demo V3.7 snapshot and makes the Thermos pressure rack react to every real attack: the Condensation side releases a compact cyan frost half-ring, the Kill-Heatwave side releases a compact amber steam half-ring, and each physical route pack absorbs the release with a short body-facing recoil. The half-rings hug the character instead of reading as a second forward attack; they are visual-only and do not add damage or control. See `docs/DEMO_V3_8_THERMOS_BACK_PRESSURE_PASS.md`.
- `marker-desire-loop.html` remains a separate three-minute Marker desire-loop experiment. Demo V3.6 absorbs its approved ownership, mounting, aim and route-light display contract, but not its three-minute balance or milestone timing. The experiment continues to isolate Copy/Archive Lv1—Lv4, one physical component install, the real line-crossing `调阅` fusion and pure-route Lv4 ultimates. See `docs/MARKER_DESIRE_LOOP_EXPERIMENT.md`.

## Authoritative development location

- Worktree: `C:\Users\Administrator\.qclaw\workspace\cubicle-foundation-test`
- Git branch: `codex/demo-v3-15-formal-20260825`
- Remote: `https://github.com/245269946-blip/Cubicle-Survivor.git`
- Latest committed formal baseline before the 2026-08-27 reliability hotfix: `5aa642a` (`Release Demo V3.15 formal cartoon build`)
- Active runnable package: `Cubicle-Survivor-demo/`
- Public GitHub Pages entry: `https://245269946-blip.github.io/Cubicle-Survivor/`
- Run QA from the active package with `npm run qa`.

The working tree may contain newer intentional UI, combat-feedback or release work and must be preserved. Always inspect `git status --short --branch` before making changes.
...
- Purchase and choice pages must stay immediately legible: objective progress, encounter closure or weapon motion should communicate the primary result before text does. Shorter copy is not sufficient when a choice still asks the player to remember several parallel system promises; simplify or defer the mechanism first.
- Treat weapon transformation and task completion as separate reward loops. Weapon growth answers what the player has become; each encounter or stage still needs to confirm what the player has just accomplished. A destructible or restorable room state is only one possible carrier, not the default product direction.
...
Weapon-visual causality guardrail (verified 2026-07-22): before changing any player-visible weapon, module, component or combat VFX, read `docs/WEAPON_VISUAL_DESIGN_STANDARD.md`. Every visible part must declare owner, socket, follow rule and state source; worn equipment must obey body-facing mount logic; only aiming entities may follow attack aim; and visible attack paths must share their origin and geometry with combat judgment. Do not use detached prop icons, arbitrary screen offsets or global recolor to substitute for physical installation and mechanic feedback.
...
V3.15 world-map and display-semantics guardrail (verified 2026-08-28): formal office scenes are 2600x1800 world maps, never viewport stage plates or parallax backdrops. The base scene and navigation overlay must move with the same camera transform as the player and enemies. Scissors fan judgment may keep two helper edges, but the renderer must coalesce those helpers into one complete scissors model; range upgrades enlarge that model and its fan area, never its model count. Light Step owns exactly one world-space ground cue rendered from measured non-transparent sprite bounds. Marker, Thermos, and Correction Fluid follow the same rule: range grows judgment-aligned geometry; only an explicit amount/module mechanic may add authored parallel outputs. See `docs/V3_15_WORLD_MAP_DISPLAY_SEMANTICS.md`.
