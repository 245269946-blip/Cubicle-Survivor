...
const weaponDefinitions = {
  ...
  shredder: {
    ...
    level: 0,
    max: 7,
    description: "围绕工位旋转碎纸刀片，清近身并提高材料收益。",
  },
  thermos: {
    label: "保温杯",
    archetype: "站桩治疗",
...
const weaponUpgradePool = [
  ...
  },
  {
    ...
    title: "桌边碎纸机",
    tag: "武器 / 近距漩涡",
    text: "获得或强化碎纸机，刀片围绕玩家切割近身目标。",
    apply: (g) => {
      g.weapons.shredder.level += 1;
      g.player.shredderBlades += 1;
    },
  ...
  },
  {
    ...
    apply: (g) => {
      g.weapons.shredder.level += 1;
      g.player.shredderCooldown *= 0.86;
      g.player.shredderSpeed += 0.55;
    },
  ...
  },
  {
    ...
    title: "加宽进纸口",
    tag: "武器 / 近距范围",
    text: "碎纸机半径和刀片伤害提升，更适合守住工位。",
    apply: (g) => {
      g.weapons.shredder.level += 1;
      g.player.shredderRadius += 12;
    },
    available: (g) => g.weapons.shredder.level >= 2 && g.weapons.shredder.level < g.weapons.shredder.max,
  },
  {
    ...
    title: "自动喷纸",
    tag: "武器 / 工程弹片",
    text: "碎纸机更容易吐出纸屑弹片，压制远处漏网压力。",
    apply: (g) => {
      g.weapons.shredder.level += 1;
      g.player.shredderPelletRate += 1;
    },
...
const statUpgradePool = [
  ...
  },
  {
    ...
    title: "碎纸机维护",
    tag: "属性 / 武器专属 近距",
    text: "碎纸机半径、转速和工程收益提升，适合贴身防御。",
    apply: (g) => {
      g.player.shredderRadius += 12;
      g.player.shredderSpeed += 0.7;
      g.player.luck += 8;
      g.player.armor += 1;
    },
    available: (g) => g.weapons.shredder.level >= 2 && (g.player.shredderRadius < 150 || g.player.armor < 26),
  },
  {
    id: "teaRoomRoutine",
    title: "茶水间节奏",
...
function createGame() {
  ...
  return {
    ...
    player: {
      ...
      orbitSpeed: 2.4,
      shredderTimer: 0,
      shredderCooldown: 0.32,
      shredderBlades: 3,
      shredderRadius: 56,
      shredderSpeed: 4.2,
      shredderKills: 0,
      shredderAngle: 0,
      shredderPelletRate: 0,
      thermosTea: 0,
      thermosTeaMax: 100,
...
function updateShredder(dt) {
  ...
  const bladeCount = getShredderBladeCount();
  const radius = getShredderRadius();
  const interval = Math.max(0.18, p.shredderCooldown - (level >= 6 ? 0.08 : 0));
  p.shredderAngle += dt * (p.shredderSpeed + bladeCount * 0.36 + (level >= 6 ? 1.4 : 0));
  p.shredderTimer -= dt;
  if (p.shredderTimer > 0) return;

  p.shredderTimer = weaponCooldown(interval / (1 + bladeCount * 0.035), "shredder");
  const damage = hitDamage((3 + level * 1.8) * (level >= 4 ? 1.25 : 1) * (1 + getClassBonus("engineering")));
  let killCount = 0;
  for (const e of game.enemies) {
    const before = e.hp;
    if (Math.hypot(e.x - p.x, e.y - p.y) < e.r + radius) {
      applyEnemyDamage(e, damage, "shredder", false);
      e.slow = Math.min(e.slow || 1, level >= 6 ? 0.72 : 0.82);
  ...
  if (killCount > 0) {
    ...
    if (level >= 3) floatingText(p.x, p.y - radius * 0.38, `碎纸 ${killCount}`, "#c5d4df");
  }
  if (level >= 5) {
    const threshold = Math.max(5, 8 - p.shredderPelletRate);
    while (p.shredderKills >= threshold) {
      p.shredderKills -= threshold;
      const angle = Math.random() * TAU;
      spawnProjectile({
        x: p.x + Math.cos(angle) * radius * 0.75,
        y: p.y + Math.sin(angle) * radius * 0.75,
        vx: Math.cos(angle) * 300,
        vy: Math.sin(angle) * 300,
        r: 3,
        life: 0.55 + rangeBonus(0.0012),
        damage: damage * (level >= 7 ? 0.82 : 0.6),
        color: "#c5d4df",
        pierce: level >= 7 ? 2 : 1,
        source: "shredder",
      });
  ...
  if (Math.random() < 0.62) {
    const angle = p.shredderAngle + Math.random() * TAU;
    spark(p.x + Math.cos(angle) * radius, p.y + Math.sin(angle) * radius, "#c5d4df");
  }
}
...
function gainXp(amount) {
  game.xp += amount * (game.policyXpMult || 1);
...
function syncWeaponDerivedStats() {
  ...
  p.shredderBlades = 3 + (shredder >= 2 ? 1 : 0) + (shredder >= 5 ? 1 : 0) + (shredder >= 7 ? 1 : 0);
  p.shredderRadius = 56 + (shredder >= 4 ? 20 : 0);
  p.shredderCooldown = 0.32 - (shredder >= 6 ? 0.06 : 0);
  p.shredderSpeed = 4.2 + Math.max(0, shredder - 1) * 0.34 + (shredder >= 6 ? 1.2 : 0);
...
function applyWeaponUpgradeModifiers() {
  ...
  p.shredderBlades += n("shredder");
  p.shredderCooldown *= Math.pow(0.86, n("shredderMotor"));
  p.shredderSpeed += n("shredderMotor") * 0.55;
  p.shredderRadius += n("shredderFeed") * 12;
  p.shredderPelletRate = n("shredderAuto");
...
function weaponCooldown(base, weaponId) {
  ...
  return Math.max(floor, base * (100 / (100 + attackSpeed * coefficient)) * hybridMult * (game.policyCooldownMult || 1));
}
...
function getShredderBladeCount() {
  const p = game.player;
  return clamp(Math.round(p.shredderBlades + Math.floor(getEffectiveStat("attackSpeed") / 55)), 3, 8);
}

function getShredderRadius() {
  return game.player.shredderRadius
    + getClassBonus("fieldRadius") * 0.3
    + Math.round(getEffectiveStat("range") * 0.18)
    + Math.round(getAnchorCharge() * 12);
}
...
function drawShredderBlades() {
  ...
  const p = game.player;
  const count = getShredderBladeCount();
  const radius = getShredderRadius();
  ctx.save();
  for (let i = 0; i < count; i += 1) {
    const angle = p.shredderAngle + (i / count) * TAU;
    const x = p.x + Math.cos(angle) * radius;
    const y = p.y + Math.sin(angle) * radius;
    ctx.translate(x, y);
    ctx.rotate(angle + Math.PI / 2);
    pixelRect(-9, -3, 18, 6, "#c5d4df");
    pixelRect(-5, -6, 10, 3, "#f4f0e8");
    pixelRect(-2, 3, 4, 5, "#7b8995");
    ctx.setTransform(1, 0, 0, 1, -game.camera.x, -game.camera.y);
  }
  ctx.restore();
  ctx.save();
  ctx.globalAlpha = 0.13;
  ctx.strokeStyle = "#c5d4df";
  ctx.lineWidth = 2;
  ctx.beginPath();
