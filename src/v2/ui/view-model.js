...
(function () {
  ...
  const WEAPON_ORDER = ["marker", "coffee", "keyboard", "thermos", "sticky_note", "stapler", "headphones", "report", "shredder", "calculator"];
  ...
  function hud(state) {
    ...
    return {
      ...
      stageNote: state.stage ? state.stage.note : "",
  ...
  function result(state) {
    ...
    return {
      title: state.flags.won ? "完成本轮验证" : "本轮结束",
      theme: pageTheme(state),
      build: buildSummary(state),
      kills: state.kills,
      level: state.level,
