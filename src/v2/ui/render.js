...
(function () {
  ...
  function renderArmory() {
    ...
    setHtml("weaponChoices", vm.offers.map(function (offer, index) {
      ...
      return '<button class="choice shop-card theme-card ' + (index === 0 ? "recommended-offer" : "") + '" type="button" data-offer="' + escapeHtml(offer.id) + '"' + (affordable ? "" : " disabled") + '>' +
        '<span class="tag route-tag">' + (index === 0 ? "优先补强" : "可选强化") + '</span>' +
  ...
  function renderPause() {
    ...
    setHtml("pauseStats", '<div>状态：' + escapeHtml(state.previousMode) + '</div><div>敌人：' + vm.enemies + '</div><div>平均帧耗时：' + vm.avgFrameMs + 'ms</div>');
  }

  function renderDebug(state) {
    const visible = state.flags.debug;
    show("debugState", visible);
    const button = el("testAllBtn");
    if (button) button.classList.toggle("hidden", !visible);
    if (!visible) return;
    setText("debugState", JSON.stringify(V2.getViewModel("debug"), null, 2));
  }
  ...
  function bindStaticControls() {
    ...
    const test = el("testAllBtn");
    if (test) test.onclick = function () {
      const result = window._testAllBuilds ? window._testAllBuilds() : [];
